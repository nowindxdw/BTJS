...
var validNamePattern = new RegExp("^[0-9a-zA-Z_]+$");//校验合法username
var validPasswordPattern = "";//校验合法password
...
exports.hash = function(str, done) {
    ...
    passhash(str).hash(function (error, hash) {
        if (error) {
            logger.error("Failt to hash the password, due to:" + error.stack);
        } else {
